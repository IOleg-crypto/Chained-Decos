...
namespace Chained
{
    ...
    template <typename T, typename Alloc> struct is_vector<std::vector<T, Alloc>> : std::true_type { using value_type = T; };
        ...
        bool Sequence(const char* name, std::vector<T>& values, bool allowAddRemove = true)
        {
            ...
            for (size_t i = 0; i < values.size(); ++i)
            {
                ...
                if (m_Archive.GetReflectionMode() == ReflectionMode::UI)
                {
                    ...
                    {
                        if (Property(label.c_str(), values[i])) changed = true;
                    }
                ...
                else
                {
                    ...
                    if constexpr (std::is_arithmetic_v<T> || std::is_same_v<T, std::string> || is_variant_v<T> ||
                                  std::is_enum_v<T> || (std::is_integral_v<T> && std::is_unsigned_v<T>))
                    {
                        if (Property(nullptr, values[i])) changed = true;
                    }
                    else if constexpr (is_rfl_component<T>::value)
                    {
