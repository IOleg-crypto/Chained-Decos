...
{
    if (!ScriptEngine::Get().GetActiveScene())
    {
        return 0;
    }
...
CH_SCRIPT_FUNC bool Entity_HasComponent(uint64_t entityID, Coral::String componentName)
{
    ...
    {
        return entity.HasComponent<AudioComponent>();
    }
    if (name == "TagComponent")
    {
...
CH_SCRIPT_FUNC Coral::Array<uint64_t> Entity_FindAllWithComponent(Coral::String componentName)
{
    ...
    auto addToVec = [&](auto view) {
        ...
        {
            ids.push_back((uint64_t)(uint32_t)entity);
        }
    };

    if (name == "TransformComponent")
    {
        addToVec(scene->GetRegistry().view<TransformComponent>());
    }
    else if (name == "RigidBodyComponent")
    {
        addToVec(scene->GetRegistry().view<RigidBodyComponent>());
    }
    else if (name == "CameraComponent")
    {
        addToVec(scene->GetRegistry().view<CameraComponent>());
    }
    else if (name == "PlayerComponent")
    {
        addToVec(scene->GetRegistry().view<PlayerComponent>());
    }
    else if (name == "AudioComponent")
    {
        addToVec(scene->GetRegistry().view<AudioComponent>());
    }
    else if (name == "TagComponent")
    {
        addToVec(scene->GetRegistry().view<TagComponent>());
    }
...
CH_SCRIPT_FUNC float PlayerComponent_GetMovementSpeed(uint64_t entityID)
{
    ...
    Entity entity((entt::entity)(uint32_t)entityID, &scene->GetRegistry());
    return entity && entity.HasComponent<PlayerComponent>() ? entity.GetComponent<PlayerComponent>().MovementSpeed
                                                            : 0.0f;
}
...
#define CH_ADD_INTERNAL_CALL(className, fieldName, funcPtr)                                                            \
    ...
    CH_ADD_INTERNAL_CALL(SceneTransitionComponent, SceneTransitionComponent_GetTargetScene_Ptr,
                         SceneTransitionComponent_GetTargetScene);
