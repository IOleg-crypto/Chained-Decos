...
Model AssetManager::LoadModel(const std::string &path)
{
    ...
    if (!fullPath.is_absolute() && path.size() > 0 && path[0] == ':')
    {
        Mesh mesh = {0};
        if (path == ":cube:")
        ...
        if (mesh.vertexCount > 0)
        {
            ...
            s_Models[path] = model;
            CH_CORE_INFO("Procedural Model Generated: %s", path);
            return model;
        }
    }
    ...
    {
        CH_CORE_ERROR("Model file does not exist: {0}", fullPath.string());
        return {0};
    }
...
Texture2D AssetManager::LoadTexture(const std::string &path)
{
    ...
    if (!std::filesystem::exists(fullPath))
    {
        CH_CORE_ERROR("Texture file does not exist: {0}", fullPath.string());
        return {0};
    }
    ...
    if (texture.id > 0)
    {
        ...
        CH_CORE_INFO("Texture Loaded: %s", path);
    }
    else
    {
        CH_CORE_ERROR("Failed to load texture: %s", path);
    }
