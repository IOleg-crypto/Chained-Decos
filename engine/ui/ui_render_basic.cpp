...
namespace Chained
{
	...
	bool RenderButton(const ButtonData& button, UIControlComponent& wc, const ImVec2& pos, const ImVec2& size,
	...
	bool RenderLabel(const LabelData& label, UIControlComponent& wc, const ImVec2& pos, const ImVec2& size,
					 ImFont* font, const TextStyle& textStyle)
	{
		ImDrawList* dl = ImGui::GetWindowDrawList();
						...
						ImFont* font, const TextStyle& textStyle)
	{
		...
		if (checkbox.Checked)
		{
			...
			dl->AddRectFilled({pos.x + pad, pos.y + pad}, {boxMax.x - pad, boxMax.y - pad},
							  ToImU32(textStyle.TextColor), wc.BoxStyle.Rounding);
		}
						   ...
						   ImFont* font, const TextStyle& textStyle)
	{
		...
		if (radio.Horizontal)
		{
			...
			for (int i = 0; i < (int)radio.Options.size(); i++)
			{
				...
				RenderAlignedTextureText(dl, font, textStyle.FontSize, radio.Options[i], labelPos, labelSize,
										 textStyle);
