...
namespace CHEngine
{
    ...
    void UIRenderer::DrawCanvas(Scene* scene, const ImVec2& referencePosition, const ImVec2& referenceSize, bool editMode)
    {
        ...
        Rectangle canvasRect = { referencePosition.x, referencePosition.y, currentRefSize.x, currentRefSize.y };
    ...
    Rectangle UIRenderer::CalculateEntityRect(Entity entity, const Rectangle& canvasRect, std::map<entt::entity, Rectangle>& rectCache)
    {
        auto& control = entity.GetComponent<ControlComponent>();
    ...
    bool UIRenderer::RenderUIComponent(Entity entity, const ImVec2& screenPos, const ImVec2& size, bool editMode)
    {
        ...
        {
            DrawPanel(entity.GetComponent<PanelControl>(), screenPos, size);
        }
            ...
            if (assetManager && !ib.TexturePath.empty()) {
                ...
                if (tex) {
                    if (ImGui::ImageButton(ib.Label.c_str(), (ImTextureID)(intptr_t)tex->GetTexture().id, size, {0,0}, {1,1}, 
                        ColorToImVec4(ib.BackgroundColor), ColorToImVec4(ib.TintColor)))
                    {
        ...
        if (entity.HasComponent<RadioButtonControl>())
        {
            ...
            for (int i = 0; i < (int)rb.Options.size(); i++)
            {
                ...
                {
                    rb.SelectedIndex = i; rb.Changed = true;
                }
