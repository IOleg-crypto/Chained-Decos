...
std::shared_ptr<Shader> Shader::Create(const std::string& vsSource, const std::string& fsSource)
{
    ...
    {
        case GraphicsDevice::API::None:    CH_CORE_ASSERT(false, "GraphicsDevice::None is currently not supported!"); return nullptr;
        case GraphicsDevice::API::OpenGL:  return std::make_shared<GLShaderProgram>(vsSource, fsSource);
    }
