...
bool RenderPlot(const PlotData& plot, UIControlComponent& wc, const ImVec2& pos, const ImVec2& size)
{
    ...
    {
        ImGui::PlotLines("##plotlines", plot.Values.data(), static_cast<int>(plot.Values.size()), 0, overlay,
                         plot.ScaleMin, plot.ScaleMax, {size.x, size.y});
    }
    else
    {
        ImGui::PlotHistogram("##plothisto", plot.Values.data(), static_cast<int>(plot.Values.size()), 0, overlay,
                             plot.ScaleMin, plot.ScaleMax, {size.x, size.y});
    }
    return false;
}
