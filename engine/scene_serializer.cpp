...
bool SceneSerializer::Deserialize(const std::string &filepath)
{
    ...
    if (!stream.is_open())
    {
        CH_CORE_ERROR("Failed to open scene file: {0}", filepath);
        return false;
    }
    ...
    CH_CORE_INFO("Deserializing scene '{0}'", sceneName);
    ...
    if (entities)
    {
        for (auto entity : entities)
        {
            ...
            CH_CORE_TRACE("Deserialized entity with ID = {0}, name = {1}", uuid, name);
            ...
            if (skyboxComponent)
            {
                ...
                sc.TexturePath = skyboxComponent["TexturePath"].as<std::string>();
                sc.Exposure = skyboxComponent["Exposure"].as<float>();
                sc.Brightness = skyboxComponent["Brightness"].as<float>();
                sc.Contrast = skyboxComponent["Contrast"].as<float>();
            }
        }
    }
