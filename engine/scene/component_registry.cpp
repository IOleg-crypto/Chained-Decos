                        ...
                        std::visit([&out](auto&& fieldValue) {
                            ...
                            else if constexpr (std::is_same_v<T, glm::vec2>)
                            {
                                out << YAML::Key << "Value" << YAML::Value
                                    << YAML::Flow << YAML::BeginSeq << fieldValue.x << fieldValue.y << YAML::EndSeq;
                            }
                            else if constexpr (std::is_same_v<T, glm::vec3>)
                            {
                                out << YAML::Key << "Value" << YAML::Value
                                    << YAML::Flow << YAML::BeginSeq << fieldValue.x << fieldValue.y << fieldValue.z << YAML::EndSeq;
                            }
                            else if constexpr (std::is_same_v<T, glm::vec4>)
                            {
                                out << YAML::Key << "Value" << YAML::Value
                                    << YAML::Flow << YAML::BeginSeq << fieldValue.x << fieldValue.y << fieldValue.z << fieldValue.w << YAML::EndSeq;
                            }
                        ...
                        if (valueNode)
                        {
                            switch (fieldType)
                            {
                                case ScriptFieldType::Float:
                                    field.Value = valueNode.as<float>(0.0f);     break;
                                case ScriptFieldType::Int:
                                    field.Value = valueNode.as<int>(0);          break;
                                case ScriptFieldType::Bool:
                                    field.Value = valueNode.as<bool>(false);     break;
                                case ScriptFieldType::String:
                                    field.Value = valueNode.as<std::string>(""); break;
                                case ScriptFieldType::Entity:
                                    field.Value = valueNode.as<uint64_t>(0);     break;
                                ...
                                case ScriptFieldType::Vec3:
                                    field.Value = (valueNode.IsSequence() && valueNode.size() >= 3)
                                        ? glm::vec3(valueNode[0].as<float>(), valueNode[1].as<float>(), valueNode[2].as<float>())
                                        : glm::vec3(0.0f);
                                    break;
        ...
        // UI — custom serializer (bypasses broken variant reflection)
        {
            ...
            metadata.Deserialize = [](Entity entity, YAML::Node node) {
                ...
                if (auto boxStyleNode = widgetNode["Box Style"]) {
                    if (boxStyleNode["BG Color"]) widgetComp.BoxStyle.BackgroundColor = boxStyleNode["BG Color"].as<Color>();
                    if (boxStyleNode["Hover Color"]) widgetComp.BoxStyle.HoverColor = boxStyleNode["Hover Color"].as<Color>();
                    if (boxStyleNode["Pressed Color"]) widgetComp.BoxStyle.PressedColor = boxStyleNode["Pressed Color"].as<Color>();
                    if (boxStyleNode["Rounding"]) widgetComp.BoxStyle.Rounding = boxStyleNode["Rounding"].as<float>();
                    if (boxStyleNode["Border Size"]) widgetComp.BoxStyle.BorderSize = boxStyleNode["Border Size"].as<float>();
                    if (boxStyleNode["Border Color"]) widgetComp.BoxStyle.BorderColor = boxStyleNode["Border Color"].as<Color>();
                    if (boxStyleNode["Gradient"]) widgetComp.BoxStyle.UseGradient = boxStyleNode["Gradient"].as<bool>();
                    if (boxStyleNode["Padding"]) widgetComp.BoxStyle.Padding = boxStyleNode["Padding"].as<float>();
                    if (boxStyleNode["Hover Scale"]) widgetComp.BoxStyle.HoverScale = boxStyleNode["Hover Scale"].as<float>();
                    if (boxStyleNode["Pressed Scale"]) widgetComp.BoxStyle.PressedScale = boxStyleNode["Pressed Scale"].as<float>();
                    if (boxStyleNode["Transition Speed"]) widgetComp.BoxStyle.TransitionSpeed = boxStyleNode["Transition Speed"].as<float>();
                }

                // Text Style
                if (auto textStyleNode = widgetNode["Text Style"]) {
                    if (textStyleNode["Font Name"]) widgetComp.TextStyle.FontName = textStyleNode["Font Name"].as<std::string>();
                    if (textStyleNode["Font Size"]) widgetComp.TextStyle.FontSize = textStyleNode["Font Size"].as<float>();
                    if (textStyleNode["Text Color"]) widgetComp.TextStyle.TextColor = textStyleNode["Text Color"].as<Color>();
                    if (textStyleNode["Shadow"]) widgetComp.TextStyle.Shadow = textStyleNode["Shadow"].as<bool>();
                    if (textStyleNode["Letter Spacing"]) widgetComp.TextStyle.LetterSpacing = textStyleNode["Letter Spacing"].as<float>();
                    if (textStyleNode["Line Height"]) widgetComp.TextStyle.LineHeight = textStyleNode["Line Height"].as<float>();
                    if (textStyleNode["H Align"]) widgetComp.TextStyle.Horizontal = static_cast<HorizontalAlignment>(textStyleNode["H Align"].as<int>());
                    if (textStyleNode["V Align"]) widgetComp.TextStyle.Vertical = static_cast<VerticalAlignment>(textStyleNode["V Align"].as<int>());
                }
