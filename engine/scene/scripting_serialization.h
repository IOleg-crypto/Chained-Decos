...
namespace YAML
{
template <> struct convert<Chained::ManagedScriptInstance>
{
    static Node encode(const Chained::ManagedScriptInstance& rhs)
    {
        Node node;
    ...
    static bool decode(const Node& node, Chained::ManagedScriptInstance& rhs)
    {
        if (node.IsScalar())
        {
        ...
        if (node["Fields"] && node["Fields"].IsSequence())
        {
            for (auto f : node["Fields"])
            {
                ...
                if (f.IsMap() && f["Type"])
                {
                    field.Type = (Chained::ScriptFieldType)f["Type"].as<int>();
                }
                ...
                if (f.IsMap() && f["Value"])
                {
                    switch (field.Type)
                    {
                    case Chained::ScriptFieldType::Float:
                        field.Value = f["Value"].as<float>();
                        break;
                    case Chained::ScriptFieldType::Int:
                        field.Value = f["Value"].as<int>();
                        break;
                    case Chained::ScriptFieldType::Bool:
                        field.Value = f["Value"].as<bool>();
                        break;
                    case Chained::ScriptFieldType::String:
                        field.Value = f["Value"].as<std::string>();
                        break;
                    case Chained::ScriptFieldType::Vec2:
                        field.Value = f["Value"].as<glm::vec2>();
                        break;
                    case Chained::ScriptFieldType::Vec3:
                        field.Value = f["Value"].as<glm::vec3>();
                        break;
                    case Chained::ScriptFieldType::Vec4:
                        field.Value = f["Value"].as<glm::vec4>();
                        break;
                    case Chained::ScriptFieldType::Color:
                        field.Value = f["Value"].as<Chained::Color>();
                        break;

                    case Chained::ScriptFieldType::Entity:
...
inline Emitter& operator<<(Emitter& out, const Chained::ManagedScriptInstance& v)
{
    out << BeginMap;
