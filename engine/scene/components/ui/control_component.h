...
namespace Chained
{
	...
	struct ControlComponent
	{
		...
		struct UI
		{
			...
			UIMeta IsActive = {
				.Tooltip = "Determines whether this UI control is updated, rendered, and processes user inputs"};
	...
	struct UIControlComponent
	{
		...
		{
			UIMeta BoxStyle = {
				.Tooltip = "Visual appearance, coloring, padding, and corner configurations for the canvas node"};
			...
			UIMeta PressedThisFrame = {.ReadOnly = true,
									   .Transient = true,
									   .Tooltip =
										   "Triggers true for a single frame immediately upon interaction click"};
