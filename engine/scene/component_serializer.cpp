...
namespace CHEngine
{
    ...
    static void deserializeUIStyle(UIStyle& style, YAML::Node node) {
        ...
        if (node["UseGradient"]) style.UseGradient = node["UseGradient"].as<bool>();
        if (node["GradientColor"]) style.GradientColor = node["GradientColor"].as<Color>();
        if (node["HoverScale"]) style.HoverScale = node["HoverScale"].as<float>();
        if (node["PressedScale"]) style.PressedScale = node["PressedScale"].as<float>();
    ...
    void ComponentSerializer::Initialize()
    {
        ...
        Register<VerticalLayoutGroup>("VerticalLayoutGroup", [](auto& archive, auto& component) {
            archive.Property("Spacing", component.Spacing)
                   .Property("Padding", component.Padding);
        });
