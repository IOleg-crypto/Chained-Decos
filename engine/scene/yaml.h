...
template <typename T, typename EnumT> inline void DecodeEnum(const Node& node, const std::string& key, EnumT& target)
{
    if (node[key])
    {
...
{
    return out << BeginMap << Key << "AnchorMin" << Value << t.AnchorMin << Key << "AnchorMax" << Value << t.AnchorMax
               << Key << "OffsetMin" << Value << t.OffsetMin << Key << "OffsetMax" << Value << t.OffsetMax << Key
               << "Pivot" << Value << t.Pivot << Key << "Rotation" << Value << t.Rotation << Key << "Scale" << Value
               << t.Scale << EndMap;
}
...
inline Emitter& operator<<(Emitter& out, const Chained::ShaderUniform& u)
{
    ...
    return out << BeginMap << Key << "Name" << Value << u.Name << Key << "Type" << Value << typeInt << Key << "Value"
               << Value << u.Value << EndMap;
}
} // namespace YAML
