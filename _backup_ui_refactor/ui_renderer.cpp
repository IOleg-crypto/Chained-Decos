        ...
        for (entt::entity entityID : sortedList)
        {
            ...
            if (entity.HasComponent<LabelControl>())
            {
                ...
                if (label.AutoSize)
                {
                    ...
                    // Update offsets to match text size
                    float targetWidth = textSize.x + 10.0f; // Minimal padding
                    float targetHeight = textSize.y + 4.0f;
                    
                    if (controlComponent.Transform.AnchorMin.x == controlComponent.Transform.AnchorMax.x &&
                        controlComponent.Transform.AnchorMin.y == controlComponent.Transform.AnchorMax.y)
                    {
                        controlComponent.Transform.OffsetMax.x = controlComponent.Transform.OffsetMin.x + targetWidth;
                        controlComponent.Transform.OffsetMax.y = controlComponent.Transform.OffsetMin.y + targetHeight;
                        
                        // Recalculate for current frame
                        rect = controlComponent.Transform.CalculateRect(
                            {parentRect.width, parentRect.height},
                            {parentRect.x, parentRect.y});
            ...
            if (entity.HasComponent<ButtonControl>())
            {
                ...
                if (button.AutoSize)
                {
                    ...
                    float targetWidth = textSize.x + button.Style.Padding * 2.0f + 10.0f;
                    float targetHeight = textSize.y + button.Style.Padding * 2.0f + 4.0f;
