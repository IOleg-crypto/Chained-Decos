...
void Animation::Update(Model &model)
{
    ...
    {
        CD_CORE_WARN("Model is null, cannot update animation");
        return;
    }
...
void Animation::SetAnimationIndex(unsigned int index)
{
    ...
    {
        CD_CORE_WARN("Animation index %u out of bounds (max %d)", index, m_animCount - 1);
    }
}
bool Animation::LoadAnimations(const std::string &path)
{
    if (path.empty())
    {
        CD_CORE_WARN("Animation path is empty");
        return false;
    }
    m_modelAnimations = LoadModelAnimations(path.c_str(), &m_animCount);
    if (m_modelAnimations == nullptr || m_animCount <= 0)
    {
        CD_CORE_WARN("No animations to load from %s", path.c_str());
        m_animCount = 0;
        return false;
    }
    return true;
