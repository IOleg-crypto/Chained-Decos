...
std::string ModelAnalyzer::GetModelNameForObjectType(int objectType, const std::string& modelName)
{
    ...
    case MapObjectType::MODEL:
        if (!modelName.empty())
        {
            TraceLog(LOG_DEBUG, "[ModelAnalyzer] MODEL object requires model: %s", modelName.c_str());
            return modelName;
        }
        TraceLog(LOG_WARNING, "[ModelAnalyzer] MODEL object has no modelName specified");
        return "";
    ...
    case MapObjectType::LIGHT:
        ...
        if (!modelName.empty())
        {
            TraceLog(LOG_DEBUG, "[ModelAnalyzer] LIGHT object (likely MODEL) requires model: %s", 
                     modelName.c_str());
            return modelName;
        }
        return "";
    ...
    case MapObjectType::PLANE:
        ...
        if (!modelName.empty())
        {
            TraceLog(LOG_DEBUG, "[ModelAnalyzer] Primitive object with custom model: %s", 
                     modelName.c_str());
            return modelName;
        }
        return "";
    ...
    default:
        TraceLog(LOG_WARNING, "[ModelAnalyzer] Unknown object type: %d", objectType);
        return "";
    }
}
...
std::vector<std::string> ModelAnalyzer::GetModelsRequiredForMap(const std::string& mapIdentifier)
{
    ...
    if (extension != ".json")
    {
        TraceLog(LOG_WARNING, "[ModelAnalyzer] Map file is not JSON format: %s", mapPath.c_str());
        return requiredModels;
    }

    TraceLog(LOG_INFO, "[ModelAnalyzer] Analyzing map for model requirements: %s", mapPath.c_str());

    // Read map file content
    std::ifstream file(mapPath);
    if (!file.is_open())
    {
        TraceLog(LOG_ERROR, "[ModelAnalyzer] Could not open map file: %s", mapPath.c_str());
        return requiredModels;
    }
    ...
    {
        TraceLog(LOG_INFO, "[ModelAnalyzer] Detected editor format map");
        AnalyzeEditorFormat(content, requiredModels);
    }
    else if (arrayStart != std::string::npos)
    {
        TraceLog(LOG_INFO, "[ModelAnalyzer] Detected game format map");
        AnalyzeGameFormat(content, requiredModels);
    }
    else
    {
        TraceLog(LOG_WARNING, "[ModelAnalyzer] No valid JSON structure found in map file");
    }

    TraceLog(LOG_INFO, "[ModelAnalyzer] Found %d required models for map", 
             static_cast<int>(requiredModels.size()));
    
    return requiredModels;
}

void ModelAnalyzer::AnalyzeEditorFormat(const std::string& content, std::vector<std::string>& requiredModels)
{
    json j;
    try
    {
    ...
    {
        TraceLog(LOG_ERROR, "[ModelAnalyzer] Error parsing editor format JSON: %s", e.what());
        return;
    }

    if (!j.contains("objects") || !j["objects"].is_array())
    {
        TraceLog(LOG_WARNING, "[ModelAnalyzer] Editor format map has no objects array");
        return;
    }
    ...
    for (const auto& object : j["objects"])
    {
        ...
        if (!modelName.empty())
        {
            ...
            {
                TraceLog(LOG_INFO, "[ModelAnalyzer] Object type %d requires model: %s (normalized from %s)",
                         objectType, normalizedName.c_str(), modelName.c_str());
            }
        }
        else if (objectType != -1)
        {
            TraceLog(LOG_DEBUG, "[ModelAnalyzer] Object type %d does not require a model", objectType);
        }
    }
}

void ModelAnalyzer::AnalyzeGameFormat(const std::string& content, std::vector<std::string>& requiredModels)
{
    size_t arrayStart = content.find("[");
            ...
            if (quoteStart != std::string::npos)
            {
                ...
                if (quoteEnd != std::string::npos)
                {
                    ...
                    if (!modelName.empty())
                    {
                        ...
                        if (AddModelIfUnique(normalizedName, requiredModels))
                        {
                            TraceLog(LOG_INFO, "[ModelAnalyzer] Found model requirement: %s (normalized from %s)",
...
bool ModelAnalyzer::AddModelIfUnique(const std::string& modelName, std::vector<std::string>& requiredModels)
{
    if (modelName.empty())
    {
        return false;
    ...
    {
        TraceLog(LOG_DEBUG, "[ModelAnalyzer] Model %s already in requirements list", modelName.c_str());
        return false;
    }
