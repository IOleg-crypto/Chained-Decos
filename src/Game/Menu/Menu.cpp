...
void Menu::Update()
{
    switch (m_state)
    {
        case MenuState::Main: m_currentMenu = m_mainMenu; break;
        case MenuState::Options: m_currentMenu = m_optionsMenu; break;
        case MenuState::GameMode: m_currentMenu = m_SetGameMode; break;
        case MenuState::Audio: m_currentMenu = m_audioMenu; break;
        case MenuState::Controls: m_currentMenu = m_controlsMenu; break;
        case MenuState::Video:
        case MenuState::Credits:
        case MenuState::Mods: break;
        default: break;
    }
...
void Menu::HandleVideoNavigation()
{
    ...
    if (IsKeyPressed(KEY_ENTER))
    {
        ...
        if (opt.label == "Back") m_state = MenuState::Options;
...
{
    if (m_state == MenuState::Credits || m_state == MenuState::Mods) return;
...
{
    if (m_action == MenuAction::None) return;
    ...
    switch (m_action)
    {
        ...
        case MenuAction::BackToMainMenu:
            if (m_state == MenuState::Video || m_state == MenuState::Audio || m_state == MenuState::Controls)
                m_state = MenuState::Options;
            else
                m_state = MenuState::Main;
            ResetAction();
        ...
        case MenuAction::ExitGame:
            ...
            if (m_engine) m_engine->RequestExit();
            ResetAction();
            break;
    ...
    for (int i = 0; i < GetScreenHeight(); ++i)
    {
        ...
        DrawLine(0, i, GetScreenWidth(), i,
                 Color{static_cast<unsigned char>(15 + 25 * t),
                       ...
                       static_cast<unsigned char>(40 + 90 * t),
                       255});
    }
...
void Menu::RenderMenu() const {
    ...
    const char *title = (m_state == MenuState::Main) ? "Chained Decos" : currentMenu[static_cast<size_t>(m_selected)].label;
    ...
    for (size_t i = 0; i < currentMenu.size(); ++i)
    {
        ...
        Rectangle btnRect = {static_cast<float>(x), static_cast<float>(y),
...
void Menu::RenderSettingsMenu()
{
    ...
    for (size_t i = 0; i < m_videoOptions.size(); i++)
    {
        ...
        DrawText(label.c_str(), 80, y, fontSize, static_cast<int>(i) == m_selected ? ORANGE : RAYWHITE);
