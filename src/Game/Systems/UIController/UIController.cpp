...
#include "Engine/Model/Utils/ModelAnalyzer.h"
...
bool UIController::Initialize(Kernel* kernel)
{
    ...
    try {
        ...
        if (m_engine) {
            ...
            auto audioService = kernel->GetService<AudioService>();
            if (audioService && audioService->audio && m_menu->GetSettingsManager()) {
                m_menu->GetSettingsManager()->SetAudioManager(audioService->audio);
                TraceLog(LOG_INFO, "[UIController] AudioManager connected to SettingsManager");
            } else {
                TraceLog(LOG_WARNING, "[UIController] Could not connect AudioManager to SettingsManager");
            }
...
void UIController::HandleMenuActions(bool* showMenu, bool* isGameInitialized)
{
    ...
    case MenuAction::StartGameWithMap:
        TraceLog(LOG_INFO, "[UIController] HandleMenuActions() - Starting HandleStartGameWithMap()");
...
void UIController::HandleSinglePlayer(bool* showMenu, bool* isGameInitialized)
{
    ...
    if (!m_menu || !m_kernel) {
        TraceLog(LOG_ERROR, "[UIController] HandleSinglePlayer() - Required services not available");
        return;
    }
    ...
    catch (const std::exception &e)
    {
        TraceLog(LOG_ERROR, "[UIController] HandleSinglePlayer() - Failed to initialize player: %s",
                 e.what());
        TraceLog(LOG_WARNING, "[UIController] HandleSinglePlayer() - Player may not render correctly");
    }
...
void UIController::EnsurePlayerSafePosition()
{
    ...
    if (!playerService || !playerService->player || !collisionService || !collisionService->cm) {
        TraceLog(LOG_ERROR, "[UIController] EnsurePlayerSafePosition() - Required services not available");
        return;
    }
    
    Player* player = playerService->player;
    CollisionManager* collisionManager = collisionService->cm;
...
{
    if (!m_kernel) {
        TraceLog(LOG_ERROR, "[UIController] ReinitializeCollisionSystemForResume() - Kernel not available");
        return;
    }
    ...
    if (!mapSystemService || !mapSystemService->mapSystem || 
        !collisionService || !collisionService->cm || 
        !modelsService || !modelsService->models) {
        TraceLog(LOG_ERROR, "[UIController] ReinitializeCollisionSystemForResume() - Required services not available");
        return;
    }
    
    MapSystem* mapSystem = mapSystemService->mapSystem;
    CollisionManager* collisionManager = collisionService->cm;
    ModelLoader* models = modelsService->models;
    
    TraceLog(LOG_WARNING, "[UIController] ReinitializeCollisionSystemForResume() - No colliders found, reinitializing...");
    std::vector<std::string> requiredModels = ModelAnalyzer::GetModelsRequiredForMap(mapSystem->GetCurrentMapPath());
    ...
    try
    {
        ...
        try
        {
            ...
            TraceLog(LOG_INFO, "[UIController] ReinitializeCollisionSystemForResume() - Resume model collisions "
                               "created successfully");
        }
        catch (const std::exception &modelCollisionException)
        {
            TraceLog(LOG_WARNING,
                     "[UIController] ReinitializeCollisionSystemForResume() - Resume model collision creation "
                     "failed: %s",
                     modelCollisionException.what());
            TraceLog(LOG_WARNING, "[UIController] ReinitializeCollisionSystemForResume() - Continuing with basic "
    ...
    {
        TraceLog(LOG_ERROR,
                 "[UIController] ReinitializeCollisionSystemForResume() - Failed to reinitialize collision system "
                 "for resume: %s",
                 e.what());
    }
}
...
void UIController::HandleResumeGame(bool* showMenu, bool* isGameInitialized)
{
    ...
    if (!modelsService || !modelsService->models ||
        !mapSystemService || !mapSystemService->mapSystem ||
        !playerSystemService || !playerSystemService->playerSystem ||
        !collisionService || !collisionService->cm) {
        TraceLog(LOG_ERROR, "[UIController] HandleResumeGame() - Required services not available");
        return;
    }
...
{
    TraceLog(LOG_INFO, "[UIController] AnalyzeMapForRequiredModels() - Analyzing map to determine required models...");
    ...
    try
    {
        ...
        {
            TraceLog(LOG_WARNING, "[UIController] AnalyzeMapForRequiredModels() - No models required for map, but player model is always needed");
            requiredModels.emplace_back("player_low"); // Always include player model
        }
    }
    catch (const std::exception &e)
    {
        TraceLog(LOG_ERROR, "[UIController] AnalyzeMapForRequiredModels() - Failed to analyze map for required models: %s", e.what());
        TraceLog(LOG_ERROR, "[UIController] AnalyzeMapForRequiredModels() - Cannot continue without model analysis");
        throw;
    }
    ...
    TraceLog(LOG_INFO, "[UIController] AnalyzeMapForRequiredModels() - Total models required: %d", requiredModels.size());
...
{
    TraceLog(LOG_INFO, "[UIController] LoadRequiredModels() - Loading required models selectively...");
    ...
    {
        TraceLog(LOG_ERROR, "[UIController] LoadRequiredModels() - Failed to load any required models");
        TraceLog(LOG_ERROR, "[UIController] LoadRequiredModels() - Cannot continue without models");
        return false;
    }
    TraceLog(LOG_INFO, "[UIController] LoadRequiredModels() - Successfully loaded %d/%d required models in %.2f seconds",
             loadResult->loadedModels, loadResult->totalModels, loadResult->loadingTime);
    return true;
}

bool UIController::InitializeCollisionSystemWithModels(const std::vector<std::string>& requiredModels)
{
    TraceLog(LOG_INFO, "[UIController] InitializeCollisionSystemWithModels() - Initializing collision system with required models...");
    
    if (!m_kernel) {
        TraceLog(LOG_ERROR, "[UIController] InitializeCollisionSystemWithModels() - Kernel not available");
        return false;
    }
    
    auto mapSystemService = m_kernel->GetService<MapSystemService>();
    if (!mapSystemService || !mapSystemService->mapSystem) {
        TraceLog(LOG_ERROR, "[UIController] InitializeCollisionSystemWithModels() - MapSystem not available");
        return false;
    }
    ...
    if (!mapSystem->InitCollisionsWithModelsSafe(requiredModels))
    {
        TraceLog(LOG_ERROR, "[UIController] InitializeCollisionSystemWithModels() - Failed to initialize collision system with required models");
        TraceLog(LOG_ERROR, "[UIController] InitializeCollisionSystemWithModels() - Cannot continue without collision system");
        return false;
    }
    TraceLog(LOG_INFO, "[UIController] InitializeCollisionSystemWithModels() - Collision system initialized successfully");
    return true;
}
...
void UIController::RegisterPreloadedModels()
{
    ...
    if (!mapSystemService || !mapSystemService->mapSystem ||
        !modelsService || !modelsService->models) {
        TraceLog(LOG_ERROR, "[UIController] RegisterPreloadedModels() - Required services not available");
        return;
    }
    ...
    {
        TraceLog(LOG_INFO,
                 "[UIController] RegisterPreloadedModels() - Registering %d preloaded models from map into ModelLoader",
        ...
        for (const auto &p : mapSystem->GetGameMap().GetMapModels())
        {
            ...
            if (loaded.meshCount > 0)
            {
                ...
                {
                    TraceLog(LOG_INFO,
                             "[UIController] RegisterPreloadedModels() - Successfully registered model from map: %s (meshCount: %d)",
                             modelName.c_str(), loaded.meshCount);
                }
                else
                {
                    TraceLog(LOG_WARNING,
                             "[UIController] RegisterPreloadedModels() - Failed to register model from map: %s",
                             modelName.c_str());
                }
            }
            else
            {
                TraceLog(LOG_WARNING,
                         "[UIController] RegisterPreloadedModels() - Skipping invalid model from map: %s (meshCount: %d)",
                         modelName.c_str(), loaded.meshCount);
            }
        }
    }
    ...
    {
        TraceLog(LOG_INFO, "[UIController] RegisterPreloadedModels() - No preloaded models in GameMap to register");
    }
}
    ...
    if (!exists)
    {
        ...
        else
        {
            ...
            for (const auto &ext : exts)
            {
                ...
                TraceLog(LOG_INFO,
                         "[UIController] AutoLoadModelIfNeeded() - Attempting to auto-load model '%s' from %s",
                ...
                if (models->LoadSingleModel(stem.empty() ? requested : stem, resourcePath, true))
                {
                    ...
                    exists = true;
                    TraceLog(LOG_INFO, "[UIController] AutoLoadModelIfNeeded() - Auto-loaded model '%s'", candidateName.c_str());
                    break;
                }
            }
        }
...
void UIController::CreateModelInstancesForMap()
{
    ...
    if (!mapSystemService || !mapSystemService->mapSystem ||
        !modelsService || !modelsService->models) {
        TraceLog(LOG_ERROR, "[UIController] CreateModelInstancesForMap() - Required services not available");
        return;
    }
    ...
    TraceLog(LOG_INFO,
             "[UIController] CreateModelInstancesForMap() - Creating model instances for array-format map (%d objects)",
    ...
    for (const auto &object : mapSystem->GetGameMap().GetMapObjects())
    {
        if (object.type == MapObjectType::MODEL && !object.modelName.empty())
        {
            ...
            {
                TraceLog(LOG_WARNING,
                         "[UIController] CreateModelInstancesForMap() - Model '%s' not available after auto-load attempts; skipping instance for object '%s'",
            ...
            {
                TraceLog(LOG_WARNING, "[UIController] CreateModelInstancesForMap() - Failed to add instance for '%s'", candidateName.c_str());
            }
            else
            {
                TraceLog(LOG_INFO, "[UIController] CreateModelInstancesForMap() - Added instance for '%s'", candidateName.c_str());
            }
        }
        else if (object.type == MapObjectType::LIGHT)
        {
            TraceLog(LOG_INFO, "[UIController] CreateModelInstancesForMap() - Skipping LIGHT object '%s' for model instance creation", object.name.c_str());
        }
    }
}
...
void UIController::LoadMapObjects(const std::string& mapPath)
{
    ...
    try
    {
        ...
        if (testFile.is_open())
        {
            ...
            if (firstLine.find("[") == 0)
            {
                TraceLog(LOG_INFO, "[UIController] LoadMapObjects() - Detected array format, using LoadGameMap");
            ...
            {
                TraceLog(LOG_INFO, "[UIController] LoadMapObjects() - Detected editor format, using LoadEditorMap");
                mapSystem->LoadEditorMap(mapPath);
            }
        }
        else
        {
            TraceLog(LOG_ERROR, "[UIController] LoadMapObjects() - Cannot open map file: %s", mapPath.c_str());
            throw std::runtime_error("Cannot open map file");
        }

        TraceLog(LOG_INFO, "[UIController] LoadMapObjects() - Map loaded successfully with %d objects", 
                 mapSystem->GetGameMap().GetMapObjects().size());
    }
    catch (const std::exception &e)
    {
...
{
    TraceLog(LOG_INFO, "[UIController] HandleStartGameWithMap() - Starting game with selected map...");
    
    if (!m_menu || !m_kernel) {
        TraceLog(LOG_ERROR, "[UIController] HandleStartGameWithMap() - Required services not available");
        return;
    }
    ...
    TraceLog(LOG_INFO, "[UIController] HandleStartGameWithMap() - Selected map: %s", selectedMapName.c_str());

    // Convert map name to full path
    std::string mapPath = ConvertMapNameToPath(selectedMapName);
    TraceLog(LOG_INFO, "[UIController] HandleStartGameWithMap() - Full map path: %s", mapPath.c_str());
    ...
    try
    {
        ...
        TraceLog(LOG_INFO, "[UIController] HandleStartGameWithMap() - Step 1 complete: Found %d required models", requiredModels.size());
    }
    catch (const std::exception &e)
    {
        TraceLog(LOG_ERROR, "[UIController] HandleStartGameWithMap() - Failed to analyze map: %s", e.what());
        return;
    }
    ...
    {
        TraceLog(LOG_ERROR, "[UIController] HandleStartGameWithMap() - Failed to load required models");
        return;
    }
    ...
    TraceLog(LOG_INFO, "[UIController] HandleStartGameWithMap() - Step 3: Initializing collision system...");
    if (!InitializeCollisionSystemWithModels(requiredModels))
    {
        TraceLog(LOG_ERROR, "[UIController] HandleStartGameWithMap() - Failed to initialize collision system");
        return;
    }
    TraceLog(LOG_INFO, "[UIController] HandleStartGameWithMap() - Step 3 complete: Collision system initialized");
    ...
    try
    {
        ...
        TraceLog(LOG_INFO, "[UIController] HandleStartGameWithMap() - Step 4 complete: Map objects loaded");
    }
    catch (const std::exception &e)
    {
        TraceLog(LOG_ERROR, "[UIController] HandleStartGameWithMap() - Failed to load map: %s", e.what());
        return;
    }
    ...
    {
        playerSystem->InitializePlayer();
        TraceLog(LOG_INFO, "[UIController] HandleStartGameWithMap() - Step 5 complete: Player initialized successfully");
    }
    catch (const std::exception &e)
    {
        TraceLog(LOG_ERROR, "[UIController] HandleStartGameWithMap() - Failed to initialize player: %s", e.what());
        TraceLog(LOG_WARNING, "[UIController] HandleStartGameWithMap() - Player may not render correctly");
    }
    ...
    *isGameInitialized = true;
    ...
    TraceLog(LOG_INFO, "[UIController] HandleStartGameWithMap() - Hiding menu and starting game...");
    HideMenuAndStartGame(showMenu);
