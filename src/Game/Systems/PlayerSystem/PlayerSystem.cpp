...
PlayerSystem::PlayerSystem()
    : m_player(nullptr),
      m_kernel(nullptr),
      m_collisionManager(nullptr), m_mapSystem(nullptr),
...
bool PlayerSystem::Initialize(Kernel* kernel)
{
    ...
    if (!collisionService || !modelsService || !audioService) {
    ...
    m_audioManager = audioService->audio;
...
void PlayerSystem::RegisterServices(Kernel* kernel)
{
    ...
    kernel->RegisterService<PlayerSystemService>(
        std::make_shared<PlayerSystemService>(this)
    );
...
void PlayerSystem::InitializePlayer()
{
    ...
    TraceLog(LOG_INFO, "[PlayerSystem] InitializePlayer() - Setting initial safe position: (%.2f, %.2f, %.2f)",
             safePosition.x, safePosition.y, safePosition.z);
    m_player->SetPlayerPosition(safePosition);

    // Setup collision and physics
    TraceLog(LOG_INFO, "[PlayerSystem] InitializePlayer() - Setting up collision manager for player...");
    ...
    TraceLog(LOG_INFO, "[PlayerSystem] InitializePlayer() - Checking if player is stuck in collision...");
    ...
    if (auto playerModel = m_models->GetModelByName("player_low"))
    {
        ...
        TraceLog(LOG_INFO, "[PlayerSystem] InitializePlayer() - Player model pointer: %p, meshCount: %d", &model,
                 model.meshCount);
        
        if (model.meshCount > 0)
        {
            m_player->SetPlayerModel(&model);
            TraceLog(LOG_INFO, "[PlayerSystem] InitializePlayer() - Player model loaded successfully.");
        }
        else
        {
            TraceLog(LOG_ERROR, "[PlayerSystem] InitializePlayer() - Player model is invalid or has no meshes");
            ...
            {
                TraceLog(LOG_ERROR,
                         "[PlayerSystem] InitializePlayer() - Failed to load player_low.glb as fallback");
            }
            else
            {
                TraceLog(LOG_INFO,
                         "[PlayerSystem] InitializePlayer() - Successfully loaded player_low.glb as fallback");
                if (auto playerModel = m_models->GetModelByName("player"))
                {
                    ...
                    if (model.meshCount > 0)
                    {
                        ...
                        TraceLog(
                            LOG_INFO,
                            "[PlayerSystem] InitializePlayer() - Player model loaded successfully with fallback.");
                    }
                }
            }
        }
    ...
    TraceLog(LOG_INFO, "[PlayerSystem] InitializePlayer() - Player initialized at (%.2f, %.2f, %.2f).",
             safePosition.x, safePosition.y, safePosition.z);
    
    // Additional safety check - ensure player is properly positioned
    Vector3 currentPos = m_player->GetPlayerPosition();
    TraceLog(LOG_INFO, "[PlayerSystem] InitializePlayer() - Player current position: (%.2f, %.2f, %.2f)",
             currentPos.x, currentPos.y, currentPos.z);
    
    // Validate player position is safe (above ground but not too high)
    if (currentPos.y < 0.0f)
    {
        TraceLog(LOG_WARNING, "[PlayerSystem] InitializePlayer() - Player position below ground level, adjusting");
        m_player->SetPlayerPosition({currentPos.x, PLAYER_SAFE_SPAWN_HEIGHT, currentPos.z});
    }
    else if (currentPos.y > 50.0f)
    {
        TraceLog(LOG_WARNING, "[PlayerSystem] InitializePlayer() - Player position too high, adjusting");
        m_player->SetPlayerPosition({currentPos.x, PLAYER_SAFE_SPAWN_HEIGHT, currentPos.z});
    }
    ...
    TraceLog(LOG_INFO, "[PlayerSystem] InitializePlayer() - Checking for player spawn objects in map...");
    ...
    {
        TraceLog(LOG_INFO, "[PlayerSystem] InitializePlayer() - No map objects or spawn zone found, using default position");
    }
    
    // Final position verification
    Vector3 finalPos = m_player->GetPlayerPosition();
    TraceLog(LOG_INFO, "[PlayerSystem] InitializePlayer() - Final player position: (%.2f, %.2f, %.2f)", finalPos.x,
...
void PlayerSystem::SavePlayerState(const std::string& currentMapPath)
{
    ...
    TraceLog(LOG_INFO, "[PlayerSystem] SavePlayerState() - Saved map path: %s", m_savedMapPath.c_str());

    m_savedPlayerPosition = m_player->GetPlayerPosition();
    m_savedPlayerVelocity = m_player->GetPhysics().GetVelocity();
    TraceLog(LOG_INFO, "[PlayerSystem] SavePlayerState() - Saved player position: (%.2f, %.2f, %.2f)",
...
void PlayerSystem::RestorePlayerState()
{
    ...
    if (!m_savedMapPath.empty())
    {
        ...
        TraceLog(LOG_INFO, "[PlayerSystem] RestorePlayerState() - Restored player position: (%.2f, %.2f, %.2f)",
                 m_savedPlayerPosition.x, m_savedPlayerPosition.y, m_savedPlayerPosition.z);
    }
