...
std::vector<MapLoader>LoadMap(const std::string path) {
     ...
     for (const auto& obj : j["objects"]) {
          ...
          mo.position = {static_cast<float>(obj["position"][0]), static_cast<float>(obj["position"][1]), static_cast<float>(obj["position"][2])};
          mo.rotation = {static_cast<float>(obj["rotation"][0]), static_cast<float>(obj["rotation"][1]), static_cast<float>(obj["rotation"][2])};
          mo.scale    = {static_cast<float>(obj["scale"][0]),    static_cast<float>(obj["scale"][1]),    static_cast<float>(obj["scale"][2])};
          ...
          mo.loadedModel.transform = MatrixMultiply(mo.loadedModel.transform, MatrixTranslate(mo.position.x, mo.position.y, mo.position.z));
