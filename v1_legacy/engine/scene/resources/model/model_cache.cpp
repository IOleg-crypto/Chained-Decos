    ...
    std::thread(
         ...
         this]() mutable
        {
            ...
            else
            {
                ...
                resultPromise.set_value(nullptr);
            }
        })
        .detach();
...
void ModelCache::PrintCacheStats() const
{
    ...
    for (const auto &[name, info] : m_cache)
    {
        usage.emplace_back(name, info.accessCount);
    }
...
std::unique_ptr<Model> ModelCache::LoadModelFromFile(const std::string &filePath)
{
    if (filePath.empty())
    {
        CD_CORE_ERROR("Empty file path for model loading");
        return nullptr;
    }
    ...
    {
        CD_CORE_ERROR("Model file not found: %s", filePath.c_str());
        return nullptr;
    }
    ...
    *model = LoadModel(filePath.c_str());
    ...
    {
        CD_CORE_ERROR("Failed to load model: %s", filePath.c_str());
        return nullptr;
    }
...
void ModelCache::ProcessLoadingQueue()
{
    while (!m_stopLoadingThreads)
    {
        ...
        {
            CD_CORE_ERROR("Failed to async load model: %s", task.modelName.c_str());
