...
void ViewportPanel::OnImGuiRender()
{
    ...
    SceneState sceneState = SceneSimulationManager::Get().GetSceneState();
    entt::entity selectedEntity = SelectionManager::Get().GetSelectedEntity();
    std::shared_ptr<CHEngine::Scene> gameScene = SceneManager::Get().GetActiveScene();
    Camera3D &camera = EditorState::Get().GetCamera().GetCamera();
    Tool currentTool = EditorState::Get().GetActiveTool();
