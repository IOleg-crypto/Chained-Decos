...
ContentBrowserPanel::ContentBrowserPanel()
    : m_RootDirectory(PROJECT_ROOT_DIR "/resources"),
      m_CurrentDirectory(PROJECT_ROOT_DIR "/resources")
{
    LoadDefaultIcons();
    RefreshDirectory();
}
...
void ContentBrowserPanel::OnAssetDoubleClicked(AssetEntry &entry)
{
    ...
    {
        SceneManager::Get().OpenScene(entry.path.string());
    }
}
...
void ContentBrowserPanel::ScanCurrentDirectory()
{
    ...
    if (ec)
    {
        CD_CORE_ERROR("[ContentBrowser] Error scanning directory: %s", ec.message().c_str());
    }
}
