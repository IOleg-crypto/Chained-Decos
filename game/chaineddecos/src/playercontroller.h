...
namespace CHEngine
{
CH_SCRIPT(PlayerController){public : CH_UPDATE(deltaTime){auto scene = GetScene();
if (!scene)
{
    return;
}
...
if (wDown)
{
    movementVector = Vector3Add(movementVector, forward);
}
if (sDown)
{
    movementVector = Vector3Subtract(movementVector, forward);
}
if (aDown)
{
    movementVector = Vector3Subtract(movementVector, right);
}
if (dDown)
{
    movementVector = Vector3Add(movementVector, right);
}
...
if (Vector3LengthSqr(movementVector) > 0.0001f)
{
    ...
    // Sync Quaternion
    transform.RotationQuat = QuaternionFromEuler(transform.Rotation.x * DEG2RAD, transform.Rotation.y * DEG2RAD,
