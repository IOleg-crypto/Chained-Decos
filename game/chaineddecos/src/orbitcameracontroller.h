...
namespace CHEngine
{
CH_SCRIPT(OrbitCameraController){public : CH_UPDATE(deltaTime){auto scene = GetScene();
if (!scene)
{
    return;
}
...
transform.RotationQuat =
    QuaternionFromEuler(transform.Rotation.x * DEG2RAD, transform.Rotation.y* DEG2RAD, transform.Rotation.z* DEG2RAD);
} // namespace CHEngine
}
;
} // namespace CHEngine
