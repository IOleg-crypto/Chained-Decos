...
namespace CHEngine
{
        ...
        std::shared_ptr<T> Get(const std::string& path)
        {
                          ...
                          std::is_same_v<T, EnvironmentAsset>)
            {
                ...
                if (asset)
                {
                    ...
                    RegisterMetadata<T>(resolved, asset->GetID());
    ...
    private:
        template <typename T>
        void RegisterMetadata(const std::string& path, AssetHandle handle)
        {
            if (m_AssetMetadata.count(handle)) return;

            AssetMetadata metadata;
            metadata.Handle = handle;
            metadata.FilePath = path;
            
            if constexpr (std::is_same_v<T, ModelAsset>) metadata.Type = AssetType::Model;
            else if constexpr (std::is_same_v<T, TextureAsset>) metadata.Type = AssetType::Texture;
            else if constexpr (std::is_same_v<T, ShaderAsset>) metadata.Type = AssetType::Shader;
            else if constexpr (std::is_same_v<T, SoundAsset>) metadata.Type = AssetType::Audio;
            else if constexpr (std::is_same_v<T, FontAsset>) metadata.Type = AssetType::Font;
            else if constexpr (std::is_same_v<T, EnvironmentAsset>) metadata.Type = AssetType::Environment;
            else metadata.Type = AssetType::None;

            m_AssetMetadata[handle] = metadata;
        }
    ...
    public:
        ...
        std::shared_ptr<T> Get(AssetHandle handle)
        {
            ...
            {
                return Get<T>(it->second.FilePath);
            }
            return nullptr;
        }
