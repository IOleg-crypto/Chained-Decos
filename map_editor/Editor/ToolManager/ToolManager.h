...
public:
    ...
    bool ExecutePendingAction(Editor &editor) override;
    void SetSelectedModel(const std::string &modelName) override;
    const std::string &GetSelectedModel() const override;
    void HandleToolInput(bool mousePressed, const Ray &ray, Editor &editor) override;

    // Update tool during drag operations
    void UpdateTool(const Ray &ray, Editor &editor) override;
...
private:
    void CreateObjectForTool(Tool tool, Editor &editor);
