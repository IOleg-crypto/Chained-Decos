...
{
    m_cameraController->SetCameraMode(CAMERA_FIRST_PERSON);
...
void Player::UpdateImpl(CollisionManager &collisionManager)
{
    ...
    if (m_movement->GetPosition().y < -100.0f)
    {
        ...
        SetPlayerPosition(m_spawnPosition);
...
void Player::Update(float deltaTime)
{
    ...
    {
        auto collService = CHEngine::Engine::Instance().GetService<::ICollisionManager>();
        m_collisionManager = std::dynamic_pointer_cast<CollisionManager>(collService);
    }
