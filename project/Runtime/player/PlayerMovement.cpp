...
void PlayerMovement::SetRotationY(float rotation)
{
...
void PlayerMovement::SetNoclip(bool enable)
{
    m_noclip = enable;
}
bool PlayerMovement::IsNoclip() const
{
    return m_noclip;
}
...
Vector3 PlayerMovement::StepMovement(const CollisionManager &collisionManager)
{
    ...
    if (m_noclip)
    {
        // In noclip mode, update position directly based on velocity, ignoring collisions and
        // gravity
...
void PlayerMovement::UpdateGrounded(const CollisionManager &collisionManager)
{
    ...
    const Vector3 offsets[] = {{0.0f, 0.0f, 0.0f},
    ...
    for (const Vector3 &off : offsets)
    {
        ...
        if (collisionManager.RaycastDown(probe, maxDistance, d, p, n))
        {
            ...
            if (m_physics.GetVelocity().y <= 0.0f)
            {
                ...
                bool withinGap =
                    (gap >= -0.3f && gap <= 1.2f); // More generous gap range for jumping
    ...
    if (!grounded && bestGap < 0.5f && bestGap >= -0.5f && m_physics.GetVelocity().y <= 0.0f)
    {
        ...
        CD_TRACE("PlayerMovement::UpdateGrounded() - Grounded via bestGap: %.2f", bestGap);
    }

    // Additional check: if we're very close to ground level (with actual collision) and not moving
    // up fast, consider grounded Only set grounded if we actually have a ground collision detected,
    // not just based on height
    if (!grounded && m_physics.GetVelocity().y <= 2.0f && bestGap < 1e8f)
    {
        ...
        // The effective ground level for the collision box should be MODEL_Y_OFFSET higher than
        // visual ground Only consider grounded if there's an actual ground hit nearby (not just low
        // Y position)
...
void PlayerMovement::SnapToGround(const CollisionManager &collisionManager)
{
    ...
    if (collisionManager.RaycastDown(center, maxDistance, hitDist, hitPoint, hitNormal))
    {
        ...
        if (gap >= 0.0f && gap <= snapThreshold)
        {
            ...
            // The collision box should be positioned so the visual model (offset by -1.0f) sits on
            // the ground
...
Vector3 PlayerMovement::ValidateCollisionResponse(const Vector3 &response,
                                                  const Vector3 &currentPosition)
{
    Vector3 validatedResponse = response;

    // Ignore small horizontal pushbacks while airborne and moving upward (BVH false positives in
    // narrow gaps)
    ...
    if (horizontalMagnitude > 1.5f)
    {
        ...
        TraceLog(
            LOG_WARNING,
            "PlayerMovement::ValidateCollisionResponse() - Clamped excessive horizontal response");
    }
