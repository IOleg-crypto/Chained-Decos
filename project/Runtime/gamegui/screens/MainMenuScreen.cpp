...
void MainMenuScreen::Render()
{
    ...
    // Title section
    ImGui::SetCursorPos(ImVec2(MenuConstants::MARGIN, MenuConstants::TOP_MARGIN - 80));

    if (GetMenu()->GetTitleFont())
        ImGui::PushFont(GetMenu()->GetTitleFont());
    ImGui::TextColored(ImVec4(1.0f, 0.75f, 0.3f, 1.0f), "CHAINED DECOS");
    if (GetMenu()->GetTitleFont())
        ImGui::PopFont();

    // Subtitle
    ImGui::SetCursorPos(ImVec2(MenuConstants::MARGIN + 5, MenuConstants::TOP_MARGIN));
    if (GetMenu()->GetMediumFont())
        ImGui::PushFont(GetMenu()->GetMediumFont());
    ImGui::TextColored(ImVec4(0.7f, 0.7f, 0.7f, 0.9f), "Industrial Parkour");
    if (GetMenu()->GetMediumFont())
        ImGui::PopFont();
