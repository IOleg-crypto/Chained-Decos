...
void SettingsManager::LoadSettings()
{
    ...
    else
    {
        TraceLog(
            LOG_WARNING,
            "SettingsManager::LoadSettings() - Could not load game.cfg, using default settings");
    }
    ...
    // Window settings will be applied by RuntimeApplication::OnConfigure() and OnStart()
    ...
    // Load video settings via ConfigManager
    m_config.GetResolution(m_targetWidth, m_targetHeight);

    // Validate loaded resolution (ensure not zero)
...
{
    // Update config with target resolution to ensure what we selected is saved,
    // even if not yet applied to window? Or use GetScreenWidth()?
    // Better to use current window size IF applied, otherwise target.
    // Actually, SaveSettings usually saves the CURRENT state.
    // Assuming Apply has been called before Save.
    m_config.SetResolution(m_targetWidth, m_targetHeight);
...
void SettingsManager::ApplyVideoSettings()
{
    ...
    if (m_targetWidth > 0 && m_targetHeight > 0)
    {
        ...
        if (GetScreenWidth() != m_targetWidth || GetScreenHeight() != m_targetHeight)
        {
            TraceLog(
                LOG_INFO,
                "SettingsManager::ApplyVideoSettings() - Changing window size from %dx%d to %dx%d",
                GetScreenWidth(), GetScreenHeight(), m_targetWidth, m_targetHeight);
            SetWindowSize(m_targetWidth, m_targetHeight);
        }
    }
    ...
    if (m_currentFpsIndex >= 0 && m_currentFpsIndex < static_cast<int>(fpsOptions.size()))
    {
        ...
        CD_INFO("SettingsManager::ApplyVideoSettings() - Setting target FPS to: %s", fps.c_str());
...
void SettingsManager::GetResolution(int &width, int &height) const
{
    width = m_targetWidth;
...
{
    m_currentAspectRatioIndex = std::max(
        0, std::min(static_cast<int>(MenuConstants::ASPECT_RATIO_OPTIONS.size()) - 1, index));
}

void SettingsManager::SetDisplayModeIndex(int index)
{
    m_currentDisplayModeIndex = std::max(
        0, std::min(static_cast<int>(MenuConstants::DISPLAY_MODE_OPTIONS.size()) - 1, index));
}

void SettingsManager::SetVSyncIndex(int index)
{
    m_currentVSyncIndex =
        std::max(0, std::min(static_cast<int>(MenuConstants::VSYNC_OPTIONS.size()) - 1, index));
}

void SettingsManager::SetFpsIndex(int index)
{
    m_currentFpsIndex =
        std::max(0, std::min(static_cast<int>(MenuConstants::FPS_OPTIONS.size()) - 1, index));
}
...
std::string SettingsManager::GetCurrentSettingValue(const std::string &settingName) const
{
    ...
    else if (settingName == "Difficulty")
    {
        ...
        if (m_gameplaySettings.difficultyLevel >= 0 &&
            m_gameplaySettings.difficultyLevel < static_cast<int>(options.size()))
        {
