...
void MapSelector::InitializeMaps()
{
    ...
    {
        CD_INFO("MapSelector::InitializeMaps() - Total maps available: %d", m_availableMaps.size());
    }
...
std::string MapSelector::GetSelectedMapName() const
{
    ...
    if (selectedMapInfo)
    {
        ...
        if (selectedMapInfo->name.find("maps/") == 0 ||
            selectedMapInfo->name.find(".json") != std::string::npos)
        {
...
void MapSelector::ScanForJsonMaps()
{
    ...
    try
    {
        ...
        CD_INFO("MapSelector::ScanForJsonMaps() - Project root directory: %s", rootDir.c_str());

        for (const std::string &dir : MAP_SEARCH_DIRECTORIES)
        {
            std::string fullDir = rootDir + dir;
            CD_TRACE("MapSelector::ScanForJsonMaps() - Checking directory: %s", fullDir.c_str());
            ...
            CD_INFO("MapSelector::ScanForJsonMaps() - Scanning directory: %s", fullDir.c_str());
            ...
            for (const auto &entry : fs::directory_iterator(fullDir))
            {
                ...
                if (isJson)
                {
                    ...
                    std::filesystem::path relPath =
                        std::filesystem::path(entry.path()).lexically_relative(rootDir);
                    std::string mapPath = relPath.generic_string(); // use forward slashes
                    // Remove any leading ./ or leading path separators
                    while (!mapPath.empty() &&
                           (mapPath.front() == '/' || mapPath.front() == '\\' ||
                            (mapPath.size() >= 2 && mapPath.substr(0, 2) == "./")))
                    {
                        if (mapPath.size() >= 2 && mapPath.substr(0, 2) == "./")
                            mapPath = mapPath.substr(2);
                        else
                            mapPath = mapPath.substr(1);
                    }
        ...
        for (size_t i = 0; i < m_availableMaps.size(); ++i)
        {
            ...
            CD_INFO("MapSelector::ScanForJsonMaps() - Map %d: %s -> %s", i, map.displayName.c_str(),
                    map.name.c_str());
        }
    }
...
void MapSelector::RenderMapSelection() const
{
    ...
    for (int i = startIndex; i < endIndex; ++i)
    {
        ...
        int x =
            (screenWidth - (MAP_BOX_WIDTH * 4 + MARGIN * 3)) / 2 + col * (MAP_BOX_WIDTH + MARGIN);
    ...
    DrawText(pageInfo.c_str(), (screenWidth - pageInfoWidth) / 2,
             startY + MAP_BOX_HEIGHT * 2 + MARGIN + 20, pageInfoFontSize, WHITE);

    // Instructions
    const char *instructions =
        "[Arrow Keys: Navigate Maps] [Enter: Select Map] [Esc: Back to Menu]";
    int instrFontSize = INSTRUCTIONS_FONT_SIZE;
    int instrWidth = MeasureText(instructions, instrFontSize);
    DrawText(instructions, (screenWidth - instrWidth) / 2, screenHeight - 40, instrFontSize,
             Fade(WHITE, 0.7f));
}
...
void MapSelector::RenderMapSelectionPanels()
{
    ...
    for (int i = startIndex; i < endIndex; ++i)
    {
        ...
        ImGui::BeginChild(("MapPanel##" + std::to_string(i)).c_str(), ImVec2(panelWidth, 180), true,
...
void MapSelector::RenderMapSelectionWindow()
{
    ...
    ImGui::BeginChild("MapSelectionWindow", ImVec2(0, -100), false,
    ...
    else
    {
        ...
        else
        {
            ...
            for (int i = startIndex; i < endIndex; ++i)
            {
                ...
                ImVec4 typeColor = map.isModelBased ? ImVec4(0.4f, 0.6f, 1.0f, 1.0f)
