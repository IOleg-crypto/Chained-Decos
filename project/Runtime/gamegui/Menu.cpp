...
void Menu::Render()
{
    // The ImGui frame is now managed by RuntimeApplication or EngineApplication
...
void Menu::OnEvent(CHEngine::Event &e)
{
    if (m_consoleManager && m_consoleManager->IsConsoleOpen())
        return;
    ...
    dispatcher.Dispatch<CHEngine::KeyPressedEvent>(
        [this](CHEngine::KeyPressedEvent &ev)
        {
            if (ev.GetKeyCode() == KEY_GRAVE)
            {
                CD_INFO("Menu::OnEvent() - Console toggle key pressed");
                ToggleConsole();
                return true;
            }
...
void Menu::SetupStyle()
{
    ...
    // Load Gantari Font in multiple sizes
    std::string fontPath =
    ...
    {
        // Add Regular (24px)
        m_fontRegular = io.Fonts->AddFontFromFileTTF(fontPath.c_str(), 24.0f);

        // Add Medium (36px)
        m_fontMedium = io.Fonts->AddFontFromFileTTF(fontPath.c_str(), 36.0f);

        // Add Title (72px)
        m_fontTitle = io.Fonts->AddFontFromFileTTF(fontPath.c_str(), 72.0f);

        if (m_fontRegular)
        {
            CD_INFO("[Menu] Loaded custom Gantari fonts");
            io.FontDefault = m_fontRegular;
        }
    }
    else
    {
        CD_WARN("[Menu] Could not find font at %s", fontPath.c_str());
    }
