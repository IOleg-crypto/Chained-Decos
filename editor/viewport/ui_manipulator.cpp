...
namespace Chained
{
	...
	bool EditorUIManipulator::OnImGuiRender(Entity selectedEntity, ImVec2 viewportPos, ImVec2 viewportSize)
	{
		...
		UIRect rect = (scene && uiRenderer) ? uiRenderer->GetEntityRect(selectedEntity) : UIRect{};
		...
		auto ProcessHandle = [&](UIHandleType type, ImVec2 pos, UIHandleType& hovered) {
			bool hoveredItem =
				(isHoveredWindow && mousePos.x >= pos.x - HANDLE_SIZE && mousePos.x <= pos.x + HANDLE_SIZE &&
			...
			if (ImGui::IsMouseDown(0))
			{
				...
				else if (m_Resizing)
				{
					...
					case UIHandleType::TopLeft:
						cc.Transform.OffsetMin = {m_StartOffsetMin.x + virtualDelta.x,
												  m_StartOffsetMin.y + virtualDelta.y};
						break;
					...
					case UIHandleType::BottomRight:
						cc.Transform.OffsetMax = {m_StartOffsetMax.x + virtualDelta.x,
												  m_StartOffsetMax.y + virtualDelta.y};
						break;
