...
namespace CHEngine
{
    ...
    void ContentBrowserPanel::RenderToolbar()
    {
        ...
        // Search Filter
        if (ImGui::InputTextWithHint("##Search", ICON_FA_MAGNIFYING_GLASS " Search...", m_FilterBuffer, sizeof(m_FilterBuffer)))
        {
            RefreshDirectory();
        }
    ...
    void ContentBrowserPanel::ScanCurrentDirectory()
    {
        ...
        for (auto &p : std::filesystem::directory_iterator(m_CurrentDirectory, ec))
        {
            ...
            if (!entry.isDirectory && m_FilterType > 0)
            {
                ...
                switch (m_FilterType)
                {
                    case 1: match = (entry.type == EditorAssetType::Scene); break;
                    case 2: match = (entry.type == EditorAssetType::Prefab); break;
                    case 3: match = (entry.type == EditorAssetType::Model); break;
                    case 4: match = (entry.type == EditorAssetType::Texture); break;
                    case 5: match = (entry.type == EditorAssetType::Script); break;
                    case 6: match = (entry.type == EditorAssetType::Audio); break;
                }
                if (!match) continue;
            }
    ...
    EditorAssetType ContentBrowserPanel::DetermineAssetType(const std::filesystem::path &path)
    {
        ...
        static const std::unordered_map<std::string, EditorAssetType> s_ExtensionMap = {
            ...
            {".chprefab", EditorAssetType::Prefab},
            {".h",       EditorAssetType::Script}, {".cpp",   EditorAssetType::Script},
            {".obj",     EditorAssetType::Model},  {".gltf",  EditorAssetType::Model}, {".glb", EditorAssetType::Model},
            {".png",     EditorAssetType::Texture}, {".jpg",   EditorAssetType::Texture}, {".tga", EditorAssetType::Texture},
            {".wav",     EditorAssetType::Audio},   {".ogg",   EditorAssetType::Audio},   {".mp3", EditorAssetType::Audio}
        };
        ...
        if (it != s_ExtensionMap.end()) return it->second;
