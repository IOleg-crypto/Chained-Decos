...
void CommandHistory::PushCommand(std::unique_ptr<IEditorCommand> command)
{
    ...
    CD_INFO("Command pushed: %s (Undo stack size: %zu)", m_UndoStack.back()->GetName().c_str(),
            m_UndoStack.size());
}
...
void CommandHistory::Undo()
{
    ...
    CD_INFO("Undoing command: %s", command->GetName().c_str());
    command->Undo();
...
void CommandHistory::Redo()
{
    ...
    CD_INFO("Redoing command: %s", command->GetName().c_str());
    command->Execute();
