...
public:
    ...
    virtual ReflectionMode GetReflectionMode() const override { return ReflectionMode::UI; }
    virtual bool HasChanged() const override { return m_Changed; }
    virtual void SetChanged(bool changed) override { m_Changed = changed; }

    // IPropertyArchive overrides
    virtual bool Property(const char* name, int& value, const PropertyMeta& meta = {}) override { return PropertyInternal(name, value, meta); }
    virtual bool Property(const char* name, float& value, const PropertyMeta& meta = {}) override { return PropertyInternal(name, value, meta); }
    virtual bool Property(const char* name, bool& value, const PropertyMeta& meta = {}) override { return PropertyInternal(name, value, meta); }
    virtual bool Property(const char* name, std::string& value, const PropertyMeta& meta = {}) override { return PropertyInternal(name, value, meta); }
    virtual bool Property(const char* name, glm::vec2& value, const PropertyMeta& meta = {}) override { return PropertyInternal(name, value, meta); }
    ...
    virtual void Header(const char* label) override { HeaderInternal(label); }
    virtual void Separator() override { SeparatorInternal(); }
    virtual bool BeginGroup(const char* label, bool defaultOpen = true) override { return BeginGroupInternal(label, defaultOpen); }
    virtual void EndGroup() override { EndGroupInternal(); }
    ...
    template <typename T_Enum> bool Property(const char* name, T_Enum& value, const char** names, int count) { return EnumPropertyInternal(name, (int&)value, names, count, {}); }
    template <typename T> bool Property(const char* name, T& value, const PropertyMeta& meta) { return PropertyInternal(name, value, meta); }
    template <typename T_Enum> bool Property(const char* name, T_Enum& value, const char** names, int count, const PropertyMeta& meta) { return EnumPropertyInternal(name, (int&)value, names, count, meta); }
        ...
        if (ImGui::TreeNodeEx(name, flags)) {
            for (size_t i = 0; i < values.size(); i++) {
                ...
                if constexpr (requires(T t, Properties<UIProperties>& p) { t.Reflect(p); }) {
                    char label[32]; sprintf(label, "Item %d", (int)i);
                    ...
                    char buf[256]; strncpy(buf, values[i].c_str(), sizeof(buf) - 1);
                ...
                ImGui::PopID();
                if constexpr (!requires(T t, Properties<UIProperties>& p) { t.Reflect(p); }) ImGui::Separator();
            }
...
private:
    template<typename T> bool PropertyInternal(const char* name, T& value, const PropertyMeta& meta) {
        ...
        if (!meta.Tooltip.empty() && ImGui::IsItemHovered()) ImGui::SetTooltip("%s", meta.Tooltip.c_str());
        UpdateState(changed);
        return changed;
    }
    ...
    void SeparatorInternal() { ImGui::Separator(); }
    bool BeginGroupInternal(const char* label, bool defaultOpen = true) {
        ...
        ImGuiTreeNodeFlags flags = ImGuiTreeNodeFlags_Framed | ImGuiTreeNodeFlags_AllowOverlap | ImGuiTreeNodeFlags_FramePadding | ImGuiTreeNodeFlags_SpanAvailWidth | ImGuiTreeNodeFlags_SpanAllColumns;
        if (defaultOpen) flags |= ImGuiTreeNodeFlags_DefaultOpen;
        bool opened = ImGui::TreeNodeEx(label, flags);
        if (ImGui::GetCurrentTable() != nullptr) ImGui::TableSetColumnIndex(1);
        return opened;
    }
    void EndGroupInternal() { ImGui::TreePop(); }
    ...
    bool StringProperty(const char* name, std::string& value, const PropertyMeta& meta) {
        if (meta.Hint == PropertyMeta::WidgetHint::Enum && std::string_view(name) == "ClassName") {
            ...
            if (options.size() > 2) std::sort(options.begin() + 1, options.end());

            int currentIndex = 0;
            for (size_t i = 1; i < options.size(); ++i) if (options[i] == value) { currentIndex = (int)i; break; }

            std::vector<const char*> optionNames;
            for (const auto& option : options) optionNames.push_back(option.c_str());
